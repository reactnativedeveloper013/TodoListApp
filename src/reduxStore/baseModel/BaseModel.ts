  ...
  export class BaseModel<P> {
    ...
    static saveAll<T extends BaseModel<{}>>(instances: T[]): void {
      ...
      instances.map(instance => {
        return saveInstance(instance, instance.getStoreKey(), instance.resource);
      });
    }
    ...
    static getBy(reference: string, value: string) {
      ...
      let filteredInstance = instances.filter((instance: any) => {
        return instance.props[reference] === value;
      });
      return filteredInstance.length ? filteredInstance[0] : null;
    }
    // isEmpty Function
    ...
    static saveInstanceArray<T extends BaseModel<{}>>(
      ...
      let instanceObj = arrayToObject(instances, identifier);
      saveInstanceArray(instanceObj, identifier);
    }
  
    static getInstance(id: string, state = store.getState()) {
      let modelState = state.models;
      if (!modelState) {
        return;
      }
